...
class AccountFundSummaryWiseTransferController extends Controller
{
    public function index(Request $request): View|Factory
    {
        ...
        $transactions = AccountTransaction::query()
            ...
            if ($transactions && count($transactions)>0) {
                return view('backend.accounting-report.transaction.fund-summary', compact('fromDate', 'toDate', 'transactions'));

            }

        return view('backend.fund_summary.index', compact('fromDate', 'toDate', 'transactions'));
    }
}

...
class ReportController extends Controller
{
	...
	public function studentAttendanceReportStatus(Request $request)
	{
		...
		if ($request->all() == []) {
			return view('backend.reports.student_attendance_report_status', compact('class_id', 'section_id', 'status'));
		} else {
			$class_id = $request->class_id;
			$section_id = $request->section_id;
			$class = get_class_name($class_id);
			...
			$report_data = DB::table('student_attendances')
				->where('attendance', $request->status)
				->get();

			return view('backend.reports.student_attendance_report_status', compact('class_id', 'section_id', 'class', 'section', 'report_data', 'status'));
		}
	}

...
class MarkController extends Controller
{
	...
	public function create(Request $request)
	{
		...
		if ($exam != "" && $class_id != "" && $section_id != "" && $subject_id != "") {
			...
			$new_fields = DB::select(
				"SELECT mark_distributions.*, mark_details.* from mark_distributions LEFT JOIN mark_details 
	...
	public function store(Request $request)
	{
		for ($i = 0; $i < count($request->student_id); $i++) {
			...
			foreach ($request->marks as $key => $value) {
				...
				$marksDt->mark_value = $value[$temp['student_id']] ?? 0;
				$marksDt->save();
			}
		}
		...
		if (!$request->ajax()) {
			return redirect('/marks/create')->with('success', _lang('Saved Successfully'));
		} else {

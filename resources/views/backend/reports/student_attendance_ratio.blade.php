            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form id="search_form" class="params-panel validate"
                        action="{{ url('reports/student_attendance_ratio/view') }}" method="post" autocomplete="off"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control monthpicker" name="month"
                                        value="{{ $month }}" readOnly="true" required>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            @foreach ($report_data as $key => $value)
                                                @php
                                                    // dd($report_data[$key]['present']);
                                                @endphp
                                                <tr>
                                                    <td>{{ $students[$key]->first_name . ' ' . $students[$key]->last_name }}
                                                    </td>
                                                    <td>{{ $students[$key]->roll }}</td>
                                                    <td>{{ $report_data[$key]['present'] }}</td>
                                                    <td>{{ $report_data[$key]['absent'] }}</td>
                                                    <td>0</td>
                                                    <td>{{ $report_data[$key]['ratio'] }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>

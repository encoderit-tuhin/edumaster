            ...
            <div class="panel panel-default no-export">
                <div class="panel-heading"><span class="panel-title">{{ _lang('List Payment Method') }}</span>
                    <a class="btn btn-primary btn-sm pull-right ajax-modal" data-title="{{ _lang('Add Payment Method') }}"
                        href="{{ route('payment_methods.create') }}">{{ _lang('Add New') }}</a>
                </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($paymentmethods as $paymentmethod)
                                <tr id="row_{{ $paymentmethod->id }}">
                                    ...
                                    <td class=''>{{ $paymentmethod->balance }}</td>
                                    <td>
                                        <form
                                            action="{{ action('PaymentMethodController@destroy', $paymentmethod['id']) }}"
                                            method="post">
                                            ...
                                            <a href="{{ action('PaymentMethodController@show', $paymentmethod['id']) }}"

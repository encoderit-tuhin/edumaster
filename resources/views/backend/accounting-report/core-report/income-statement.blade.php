    ...
    @if (isset($ledgers) && !empty($ledgers) && count($ledgers) > 0)
        ...
        <div class="card-border">
            <div class="card-header py-3 d-flex justify-content-between">
                <div>
                    <p> Income Statement <strong>{{ $fromDate }}</strong>
                    </p>
                </div>
            ...
            <div class="card-body">
                        ...
                        @foreach ($income as $ledger)
                            <tr>
                                <td colspan="4" class="ledger-details">
                                    ...
                                    <a
                                        href="{{ route('income-statement-details', ['from_date' => $fromDate, 'to_date' => $toDate, 'ledger_id' => $ledger->ledger_id]) }}">{{ $ledger->ledger_name }}</a>
                ...
                <table class="table table-bordered data-table">
                    <thead>
                        ...
                        {{-- <th colspan="4"> {{ $totalDebit}}</th>
                        <th colspan="4">{{ $totalCredit}}</th> --}}
                        {{-- <th colspan="4">{{ $totalDebit-$totalCredit}}</th>
                        <th colspan="4">{{ $totalDebit-$totalCredit}}</th> --}}
                        {{-- <th colspan="4">{{ _lang('Ledger-Profite/Loss') }}</th> --}}

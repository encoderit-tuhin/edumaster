        ...
        <div class="col-md-12">
            <div class="panel panel-default" data-collapsed="0">
                ...
                <div class="panel-body">
                    <form id="search_form" class="params-panel validate" action="{{ url('marks/create') }}" method="post"
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="subject_id" id="subject_id" class="form-control select2"
                                        onChange="getExam();" required>
                                        <option value="">{{ _lang('Select One') }}</option>
                                        {{ create_option('subjects', 'id', 'subject_name', $subject_id, ['class_id = ' => $class_id]) }}
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <button type="submit" style="margin-top:24px;"
                                    class="btn btn-primary btn-block rect-btn">{{ _lang('Marks') }}</button>
                            </div>
                        </div>
                    </form>
                    ...
                    @if (!empty($marks))
                        <div class="col-md-12">
                            ...
                            <form method="post" action="{{ url('marks/store') }}" class="appsvan-submit"
                                autocomplete="off" accept-charset="utf-8">
                                @csrf
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($marks as $key => $data)
                                            <tr>
                                                ...
                                                @if (empty($mark_details))
                                                    @foreach ($mark_destributions as $md)
                                                        <td><input type="text" class="form-control float-field"
                                                                name="marks[{{ $md->mark_distribution_type }}][{{ $data->student_id }}]"
                                                                value=""></td>
                                                    @endforeach
                                                @else
                                        ...
                                        <tr>
                                            <td colspan="100"><button type="submit"
                                                    class="btn btn-primary pull-right rect-btn">{{ _lang('Save Marks') }}</button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>

        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading"><span class="panel-title">{{ _lang('List Grade') }}</span>
                    <a class="btn btn-primary btn-sm pull-right ajax-modal" data-title="{{ _lang('Add Grade') }}"
                        href="{{ route('grades.create') }}">{{ _lang('Add New') }}</a>
                </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($grades as $grade)
                                <tr id="row_{{ $grade->id }}">
                                    ...
                                    <td>
                                        <form action="{{ action('GradeController@destroy', $grade['id']) }}"
                                            method="post">
                                            <a href="{{ action('GradeController@edit', $grade['id']) }}"
                                                data-title="{{ _lang('Update Grade') }}"
                                                class="btn btn-warning btn-sm ajax-modal">{{ _lang('Edit') }}</a>
                                            <a href="{{ action('GradeController@show', $grade['id']) }}"
                                                data-title="{{ _lang('View Grade') }}"

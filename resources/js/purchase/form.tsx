...
export default function PurchaseForm({ purchase }: { purchase: ITransaction }) {
  const suppliers = window?.suppliers ?? [];
  console.log("suppliers", suppliers);
  const items = window?.items ?? [];
  const removePurchaseItemFromListRoute = window?.removePurchaseItemFromList;
...
console.log('purchase', purchase)
  ...
  const handleAddItem = () => {
    ...
    if (formData.item_id && formData.price && formData.quantity) {
      ...
      const newItem: IPurchaseFormData = {
  ...
  return (
    <div>
      <div className="panel-body row">
        ...
        >
          {purchase?.id && <input type="hidden"  name="_method" value="PATCH" />}
          {ReactHtmlParser(window?.csrf_field)}
          <div className="col-md-6 mb-4">
            <div className="shadow p-3">
              <h6 className="text-center">Purchase Form</h6>
